...
class Source:
    ...
    def arrival_event(self, fac):
        ...
        #release the order in batch
        for i in range(order_num):
            if self.order_info.loc[i, "arrival_time"] == T_NOW:
        ...
        #dispatching and start processing
        for mc in fac.machines.values():
            mc.start_processing()
...
class Factory:
    ...
    def next_event(self, stop_time):
        ...
        while T_NOW < stop_time:
            ...
            T_LAST     = T_NOW
            event_type_last = event_type
            T_NOW      = self.event_lst.min()["time"]
            event_type = self.event_lst['time'].astype(float).idxmin()
            # if T_LAST == T_NOW:
            #     if event_type != event_type_last:
