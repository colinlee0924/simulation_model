...
#entity
class Job:
    def __init__(self, ID, jtype, routing, AT, DD):
        self.ID = ID
        self.jtype = jtype
...
#resource in factory
class Source:
    def __init__(self, fac, job_info, AV, DD_factor, routing_info):
        self.fac = fac
    ...
    def arrival(self):
        ...
        while True:
            ...
            arrival_interval = self.AV_interval
            ...
            routing = self.routing_info.loc[jtype, "routing"].split(',')
            AT    = self.env.now
            DD    = AT + self.DD_factor
            job   = Job(ID, jtype, routing, AT, DD)
...
#factory
class Factory:
    def __init__(self, env, job_info, routing_info, AV, DD, PT_table, ST_table, ME_table, DP_rule, log = True):
        self.env = env
        self.log = log
        self.AV_interval = AV
        self.DD_factor = DD
    ...
    def initialize(self):
        #build
        self.source = Source(self, self.job_info, self.AV_interval, self.DD_factor, self.routing_info)
...
if __name__ == '__main__':
    ...
    output_dir = os.getcwd() + '/output_result/'
    ...
    #parameter
    simulationTime =  300
    routing_info = pd.read_excel(input_dir + "routing_info.xlsx", index_col='type')
    ...
    fac = Factory(env, job_info, 5, 100, PT_table, routing_info, ST_table, ME_table, DP_rule, log = True)
    fac.initialize()
    #run simulation
